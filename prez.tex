...
\begin{frame}{Użyte technologie}
  \begin{itemize}
    ...
    \item JavaScript: \texttt{vanilla js}, \texttt{tailwind css},
      \texttt{leaflet.js}
    \item SQLite
    \item nginx
    \item API: Duffel, Geoapify (\texttt{httpx async})
  \end{itemize}
\end{frame}
\begin{frame}{Demo}
  \begin{itemize}
...
\begin{frame}{Zbieranie danych o lotach}
  \begin{itemize}
    ...
    \item Dane w naszej bazie zawsze będą trochę do tyłu względem
      ...
      01.07.2026--07.07.2026 są ekstrapolowane na tydzień
...
\begin{frame}{Zbieranie danych o atrakcjach}
  \begin{itemize}
    \item Zbieranie danych o atrakcjach dzieje się asynchronicznie
      podczas działania algorytmu. Dane są sczytywane z bazy
      statycznej, gdzie mamy ich dane oraz zdjęcia.
...
\begin{frame}{Algorytm}
  \begin{itemize}
    \item Interesują nas tylko te drogi, które są Pareto optymalne,
      oraz które wychodzą i wracają do
      wyznaczonego miasta, przelatując przy tym przez wszystkie miasta,
...
\begin{frame}{Walidacja}
  \begin{itemize}
    \item Znalezione drogi są \emph{walidowane}, tzn. sprawdzamy, czy
      sprawdzamy ich dostępność oraz zmieniamy ceny na aktualne.
...
\begin{frame}{Co mogliśmy zrobić lepiej?}
  \begin{itemize}
    \item Za dużo czasu zajęło nam znalezienie odpowiedniego dostawcy
      danych lotów.
    \item Niektóre częsci kodu są nadmiernie złożone.
    \item Złożoność zasadniczej części alogytmu jest duża, jeżeli
      miast które użytkownik chce odwiedzić jest dużo. Padł pomysł,
      aby tę część kodu przepisać na \texttt{C}, ale nie było na to czasu.
  \end{itemize}
\end{frame}
\begin{frame}{Podsumowanie}
  Czy wypełniliśmy cele? \emph{Tak.}

  Jest to nasz pierwszy \emph{akademicki} projekt, którego, naszym
  zdaniem, nie powinnyśmy się wstydzić.
\end{frame}
\begin{frame}
  \centering
  \Huge Pytania?
